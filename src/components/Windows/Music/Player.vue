    ...
    <div class="flex items-center justify-between h-full px-2">
      <div class="w-1/3">
        <div class="flex items-center gap-2">
          <img v-if="currentTrack.album && currentTrack.album.images && currentTrack.album.images.length > 0" :src="currentTrack.album.images[2].url" alt="Track Image" class="w-10 h-10">
          <div>
            <p class="text-xs font-trebuchet-pixel">{{ currentTrack.name }}</p>
            <p class="text-xs font-trebuchet-pixel">{{ currentTrack.artists && currentTrack.artists.length > 0 ? currentTrack.artists[0].name : '' }}</p>
          </div>
        </div>
      </div>
      <div class="w-1/3 flex items-center justify-center">
        ...
        <button @click="togglePlay" class="w-10 h-10 flex items-center justify-center rounded-full bg-white hover:bg-gray-200 relative overflow-hidden play-button" :class="{ 'is-playing': isPlaying }"></button>
  ...
  async function loadTrack(device_id, trackUri) {
    ...
    try {
      const response = await fetch(`https://api.spotify.com/v1/me/player/play?device_id=${device_id}`, {
        method: 'PUT',
        body: JSON.stringify({ uris: [trackUri], play: false }), // Ensure play is set to false
        headers: {
